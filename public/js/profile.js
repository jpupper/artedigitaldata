...
function renderEventoCard(ev, isDoorEvent) {
  const youtubeId = extractYouTubeId(ev);
  const borderColor = isDoorEvent ? 'border-cyan-500/20 hover:border-cyan-500/40' : 'border-magenta-500/10 hover:border-magenta-500/30';
  const dateColor = isDoorEvent ? 'bg-cyan-500/20 text-cyan-400' : 'bg-magenta-500/20 text-magenta-400';
  return `
    <div class="rounded-2xl p-5 border ${borderColor} bg-white/5 transition-all flex flex-col h-full group">
      <div class="relative aspect-video w-full overflow-hidden rounded-xl mb-4">
         <div class="block w-full h-full relative cursor-pointer"
            ${youtubeId ? `onmouseenter="playVideo(this, '${youtubeId}')" onmouseleave="stopVideo(this)"` : ''}
      ...
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-lg ${dateColor} flex flex-col items-center justify-center font-bold shrink-0">
        ...
        <div class="flex gap-1 shrink-0">
          ${isDoorEvent ? `
            <a href="${CONFIG.BASE}/event-tickets.html?event=${ev._id}" class="px-2 py-1 rounded-lg bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 transition-all text-xs font-bold flex items-center gap-1" title="Administrar entradas"><i class="fas fa-door-open"></i><span>Puerta</span></a>
          ` : ''}
          ${(!isDoorEvent && isLoggedIn() && (isOwner || isAdmin())) ? `
            ${ev.ticketConfig?.enabled ? `<a href="${CONFIG.BASE}/event-tickets.html?event=${ev._id}" class="text-green-400 hover:text-green-300 transition-colors p-1" title="Administrar entradas"><i class="fas fa-ticket-alt text-xs"></i></a>` : ''}
...
window.renderUserEventos = function(eventos, doorEvents) {
  const container = document.getElementById('user-eventos');
  if (!container) return;

  const hasDoorEvents = doorEvents && doorEvents.length > 0;
  const hasOwnEvents = eventos && eventos.length > 0;

  if (!hasOwnEvents && !hasDoorEvents) {
  ...
  if (hasOwnEvents) {
    if (hasDoorEvents) {
      html += `<div class="col-span-full mb-2"><h3 class="text-xs font-black text-gray-500 uppercase tracking-widest flex items-center gap-2"><i class="fas fa-calendar-alt text-magenta-500"></i>Mis Eventos</h3></div>`;
    }
    html += eventos.map(ev => renderEventoCard(ev, false)).join('');
  }
    ...
    html += `
      <div class="col-span-full mt-6 mb-2">
        <h3 class="text-xs font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
          <i class="fas fa-door-open text-cyan-500"></i>Control de Acceso / Puerta
        </h3>
        <p class="text-xs text-gray-600 mt-1">Eventos donde podés escanear y canjear entradas</p>
      </div>
    `;
    html += doorEvents.map(ev => renderEventoCard(ev, true)).join('');
  }
