...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Controles específicos de Modo CODE
  const codeMouseRetriggerToggle = document.getElementById('param-CODE_MOUSE_RETRIGGER');
  if (codeMouseRetriggerToggle) {
    codeMouseRetriggerToggle.addEventListener('change', () => {
      applyConfigChange('CODE_MOUSE_RETRIGGER', codeMouseRetriggerToggle.checked);
    });
  }

  const codeGlitchCharsInput = document.getElementById('param-CODE_GLITCH_CHARS');
  if (codeGlitchCharsInput) {
    codeGlitchCharsInput.addEventListener('input', () => {
      applyConfigChange('CODE_GLITCH_CHARS', codeGlitchCharsInput.value);
    });
  }
  ...
  function renderFlyerWordsList() {
    ...
    flyerWords.forEach((item, idx) => {
      ...
      pill.addEventListener('dblclick', (e) => {
        e.stopPropagation();
        const currentTxt = itemObj.text || itemObj.word || '';
        const newText = prompt('Editar texto de la palabra del flyer:', currentTxt);
        if (newText && newText.trim()) {
            ...
            timelineLayers.forEach(layer => {
              if (Array.isArray(layer.clips)) {
                layer.clips.forEach(c => {
                  if (c.id === itemObj.id || c.flyerId === itemObj.id) {
                    c.text = upper;
                    c.word = upper;
                  }
                });
          ...
          if (window.updateFlyerWordParticles) {
            window.updateFlyerWordParticles(itemObj.id, { text: upper, visible: true });
          }
  ...
  let selectedFlyerLetterIndex = null;

  function hexToRgbArr(hex) {
    if (!hex || typeof hex !== 'string') return [64, 196, 255];
    const clean = hex.replace('#', '');
    const num = parseInt(clean.length === 3 ? clean.split('').map(c => c + c).join('') : clean, 16);
    if (isNaN(num)) return [64, 196, 255];
  ...
  function interpolatePalette(paletteColors, t) {
    ...
    const frac = scaled - i;
    const c1 = hexToRgbArr(paletteColors[i]);
    const c2 = hexToRgbArr(paletteColors[i + 1]);
  ...
  function applyWordPaletteGradient(matchingWord) {
    ...
    if (window.updateFlyerWordParticles) {
      window.updateFlyerWordParticles(matchingWord.id, {
        x: matchingWord.x,
        y: matchingWord.y,
  ...
  function applyLetterColorChange(newColor) {
    ...
    if (selectedFlyerLetterIndex !== null) {
      ...
      if (window.updateFlyerWordParticles) {
        window.updateFlyerWordParticles(matchingWord.id, {
          ...
          y: matchingWord.y,
          letterColors: matchingWord.letterColors,
          letterColor: { index: selectedFlyerLetterIndex, color: newColor }
        });
      }
    } else {
      matchingWord.color = newColor;
      matchingWord.letterColors = null;
      if (window.updateFlyerWordParticles) {
        window.updateFlyerWordParticles(matchingWord.id, {
          x: matchingWord.x,
          y: matchingWord.y,
  ...
  function renderWordLettersChips(wordItem) {
    const container = document.getElementById('word-letters-chips');
    const badge = document.getElementById('selected-letter-badge');
    ...
    if (!wordItem) {
      if (badge) badge.textContent = 'Ninguna palabra';
      return;
    }

    const text = (typeof wordItem === 'object' && (wordItem.text || wordItem.word)) ? (wordItem.text || wordItem.word) : String(wordItem);
    const chars = Array.from(String(text).trim().toUpperCase());

    // Chip "Toda la palabra"
    const allChip = document.createElement('button');
    allChip.type = 'button';
    allChip.className = 'letter-chip' + (selectedFlyerLetterIndex === null ? ' active' : '');
    allChip.innerHTML = `<i class="fas fa-layer-group"></i> Toda`;
    allChip.title = 'Seleccionar y pintar toda la palabra';
    allChip.addEventListener('click', (e) => {
      e.stopPropagation();
      selectedFlyerLetterIndex = null;
      renderWordLettersChips(wordItem);
      if (letterColorInput && wordItem.color) {
        letterColorInput.value = wordItem.color;
      }
    });
    container.appendChild(allChip);

    // Chips individuales de letras
    chars.forEach((char, idx) => {
      ...
      chip.innerHTML = `
        ...
        <span>${char}</span>
        <span style="font-size: 9px; opacity: 0.6;">#${idx + 1}</span>
      `;
      chip.title = `Seleccionar letra '${char}' (posición ${idx + 1})`;
      chip.addEventListener('click', (e) => {
        e.stopPropagation();
        selectedFlyerLetterIndex = idx;
        renderWordLettersChips(wordItem);
        if (letterColorInput) {
          letterColorInput.value = charColor;
        }
      });
    ...
    if (badge) {
      if (selectedFlyerLetterIndex === null) {
        badge.textContent = 'Toda la palabra';
  ...
  function setFormationMode(mode, save = true) {
    const validMode = (mode === 'CODE') ? 'CODE' : 'FISICS';

    document.querySelectorAll('[data-mode="FISICS"]').forEach(b => {
      if (validMode === 'FISICS') b.classList.add('active');
      else b.classList.remove('active');
    });
    document.querySelectorAll('[data-mode="CODE"]').forEach(b => {
      if (validMode === 'CODE') b.classList.add('active');
      else b.classList.remove('active');
    });

    // Visibilidad dinámica de parámetros: Físicas solo en FISICS, Code solo en CODE
    const fisicsGroups = document.querySelectorAll('.group-mode-fisics');
    const codeGroups = document.querySelectorAll('.group-mode-code');

    if (validMode === 'FISICS') {
      fisicsGroups.forEach(el => { el.style.display = ''; });
      codeGroups.forEach(el => { el.style.display = 'none'; });
    } else {
      fisicsGroups.forEach(el => { el.style.display = 'none'; });
      codeGroups.forEach(el => { el.style.display = ''; });
    }
    ...
    if (save && window.ParticlesConfig && typeof window.ParticlesConfig.save === 'function') {
      window.ParticlesConfig.save({ FORMATION_MODE: validMode });
    }
  }
  window.setFormationMode = setFormationMode;

  function initFormationModeSwitch() {
    const fisicsBtn = document.getElementById('panel-btn-mode-fisics');
    const codeBtn = document.getElementById('panel-btn-mode-code');

    if (fisicsBtn) {
      fisicsBtn.addEventListener('click', (e) => {
        e.preventDefault();
        setFormationMode('FISICS', true);
      });
    }
    ...
    if (codeBtn) {
      codeBtn.addEventListener('click', (e) => {
        e.preventDefault();
        setFormationMode('CODE', true);
      });
    }

    const currentMode = (window.ParticlesConfig && window.ParticlesConfig.get().FORMATION_MODE) || 'FISICS';
  ...
  function addFlyerWord(rawText, x, y) {
    ...
    const activeFormationMode = (window.ParticlesConfig && window.ParticlesConfig.get().FORMATION_MODE) || 'FISICS';
    ...
    renderFlyerWordsList();
    renderTimelineTracks();
    updateUIForSelectedLayerProps(wordItem);
    renderWordLettersChips(wordItem);

    if (flyerWordInput) {
      flyerWordInput.value = cleanWord;
      flyerWordInput.focus();
      flyerWordInput.select();
    }
  }
  ...
  if (flyerWordInput) {
    flyerWordInput.addEventListener('input', () => {
      const activeId = window.activeFlyerWordId || selectedFlyerWordId || selectedClipId;
      if (!activeId || !flyerWords || flyerWords.length === 0) return;

      const newUpper = flyerWordInput.value.toUpperCase();
      const targetWord = flyerWords.find(w => typeof w === 'object' && w.id === activeId);
      if (targetWord) {
        const textToSet = newUpper || ' ';
        targetWord.text = textToSet;
        targetWord.word = textToSet;
        targetWord.name = textToSet;

        // Sincronizar en timelineLayers
        if (Array.isArray(timelineLayers)) {
          timelineLayers.forEach(layer => {
            if (Array.isArray(layer.clips)) {
              layer.clips.forEach(c => {
                if (c.id === activeId || c.flyerId === activeId) {
                  c.text = textToSet;
                  c.word = textToSet;
                }
              });
        ...
        // Actualizar partículas en pantalla en vivo
        if (window.updateFlyerWordParticles) {
          window.updateFlyerWordParticles(activeId, {
            text: textToSet,
            x: targetWord.x,
            y: targetWord.y,
            fontSize: targetWord.fontSize,
  ...
  function updateUIForSelectedLayerProps(props) {
    ...
    if (props.color) {
      const letterColorInput = document.getElementById('param-SELECTED_LETTER_COLOR');
      if (letterColorInput) letterColorInput.value = props.color;
    }
    if (props.palette && Array.isArray(props.palette)) {
      ['param-COLOR_1', 'param-COLOR_2', 'param-COLOR_3', 'param-COLOR_4'].forEach((id, idx) => {
        const el = document.getElementById(id);
        if (el && props.palette[idx]) el.value = props.palette[idx];
      });
    }
  ...
  function applySelectionPropChange(propName, value, globalPropName) {
    const selectedIds = (typeof window.getSelectedFlyerWordIds === 'function')
      ...
      : (selectedFlyerWordId ? [selectedFlyerWordId] : (window.activeFlyerWordId ? [window.activeFlyerWordId] : []));

    const keyMap = {
      'WORD_BOX_FILL_COLOR': 'boxFillColor',
      'WORD_BOX_FILL_OPACITY': 'boxFillOpacity',
      'WORD_BOX_STROKE_COLOR': 'boxStrokeColor',
      'WORD_BOX_STROKE_WIDTH': 'boxStrokeWidth',
      'WORD_BOX_LINE_PATTERN': 'boxLinePattern',
      'WORD_BOX_ROUNDING': 'boxRounding',
      'WORD_BOX_PADDING_X': 'boxPaddingX',
      'WORD_BOX_PADDING_Y': 'boxPaddingY',
      'WORD_BOX_PADDING': 'boxPadding',
      'WORD_BOX_ENABLED': 'boxEffectEnabled',
      'WORD_BOX_SPEED': 'boxSpeed'
    };

    const camelKey = keyMap[propName] || propName;
    const configKey = globalPropName || (Object.keys(keyMap).find(k => keyMap[k] === propName) || propName);

    if (Array.isArray(flyerWords) && selectedIds.length > 0) {
      flyerWords.forEach(w => {
        if (selectedIds.includes(w.id)) {
          w[camelKey] = value;
          if (camelKey === 'boxPaddingX' || camelKey === 'boxPaddingY') {
            w.boxPadding = value;
          }
          if (window.updateFlyerWordParticles) {
            window.updateFlyerWordParticles(w.id, { [camelKey]: value });
          }
        }
      });
    }
    ...
    if (configKey) {
      applyConfigChange(configKey, value);
    }
    ...
    timelineLayers.forEach((layerObj, layerIndex) => {
      ...
      layerObj.clips.forEach(clipObj => {
        ...
        const handleClipStart = (clientX, clientY) => {
          ...
          selectedFlyerWordId = clipObj.id;
  ...
  // Color picker de Letra Seleccionada
  const selectedLetterColorEl = document.getElementById('param-SELECTED_LETTER_COLOR');
  if (selectedLetterColorEl) {
    selectedLetterColorEl.addEventListener('input', () => {
  ...
  window.syncSelectionSubtabValues = function() {
    ...
    const syncCheck = (id1, id2, checkedVal) => {
      [id1, id2].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.checked = Boolean(checkedVal);
      });
    };

    const syncVal = (id1, id2, val) => {
      [id1, id2].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = val;
      });
    ...
    const padX = sourceObj ? ((sourceObj.boxPaddingX !== undefined) ? sourceObj.boxPaddingX : ((sourceObj.boxPadding !== undefined) ? sourceObj.boxPadding : ((cfg.WORD_BOX_PADDING_X !== undefined) ? cfg.WORD_BOX_PADDING_X : 16))) : ((cfg.WORD_BOX_PADDING_X !== undefined) ? cfg.WORD_BOX_PADDING_X : 16);
    const padY = sourceObj ? ((sourceObj.boxPaddingY !== undefined) ? sourceObj.boxPaddingY : ((sourceObj.boxPadding !== undefined) ? sourceObj.boxPadding : ((cfg.WORD_BOX_PADDING_Y !== undefined) ? cfg.WORD_BOX_PADDING_Y : 16))) : ((cfg.WORD_BOX_PADDING_Y !== undefined) ? cfg.WORD_BOX_PADDING_Y : 16);
    const speed = sourceObj ? ((sourceObj.boxSpeed !== undefined) ? sourceObj.boxSpeed : ((cfg.WORD_BOX_SPEED !== undefined) ? cfg.WORD_BOX_SPEED : 3)) : ((cfg.WORD_BOX_SPEED !== undefined) ? cfg.WORD_BOX_SPEED : 3);

    syncCheck('param-WORD_BOX_EFFECT_ENABLED', 'param-WORD_BOX_ENABLED', isEnabled);
    syncVal('param-WORD_BOX_FILL_COLOR', 'param-GLOBAL_WORD_BOX_FILL_COLOR', fillColor);
    syncVal('param-WORD_BOX_FILL_OPACITY', 'param-GLOBAL_WORD_BOX_FILL_OPACITY', fillOpacity);
    syncVal('num-WORD_BOX_FILL_OPACITY', 'num-GLOBAL_WORD_BOX_FILL_OPACITY', fillOpacity);
    syncVal('param-WORD_BOX_STROKE_COLOR', 'param-GLOBAL_WORD_BOX_STROKE_COLOR', strokeColor);
    syncVal('param-WORD_BOX_STROKE_WIDTH', 'param-GLOBAL_WORD_BOX_STROKE_WIDTH', strokeWidth);
    syncVal('num-WORD_BOX_STROKE_WIDTH', 'num-GLOBAL_WORD_BOX_STROKE_WIDTH', strokeWidth);
    syncVal('param-WORD_BOX_LINE_PATTERN', 'param-GLOBAL_WORD_BOX_LINE_PATTERN', linePattern);
    syncVal('param-WORD_BOX_ROUNDING', 'param-GLOBAL_WORD_BOX_ROUNDING', rounding);
    syncVal('num-WORD_BOX_ROUNDING', 'num-GLOBAL_WORD_BOX_ROUNDING', rounding);
    syncVal('param-WORD_BOX_PADDING_X', 'param-GLOBAL_WORD_BOX_PADDING_X', padX);
    syncVal('num-WORD_BOX_PADDING_X', 'num-GLOBAL_WORD_BOX_PADDING_X', padX);
    syncVal('param-WORD_BOX_PADDING_Y', 'param-GLOBAL_WORD_BOX_PADDING_Y', padY);
    syncVal('num-WORD_BOX_PADDING_Y', 'num-GLOBAL_WORD_BOX_PADDING_Y', padY);
    syncVal('param-WORD_BOX_SPEED', 'param-WORD_BOX_SPEED', speed);
    syncVal('num-WORD_BOX_SPEED', 'num-WORD_BOX_SPEED', speed);
  };

  const bindDualControl = (ids, propName, globalPropName, eventType = 'input', parseFn = null) => {
    ids.forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      el.addEventListener(eventType, () => {
        ...
        applySelectionPropChange(propName, val, globalPropName);
      });
    });
  };

  const bindDualCombo = (sliderIds, numIds, propName, globalPropName, parseFn = parseFloat) => {
    const handleInput = (val, sourceEl) => {
      sliderIds.concat(numIds).forEach(id => {
        const el = document.getElementById(id);
        if (el && el !== sourceEl) el.value = val;
      });
      applySelectionPropChange(propName, val, globalPropName);
    };
    ...
    sliderIds.forEach(id => {
      const el = document.getElementById(id);
      if (el) {
        el.addEventListener('input', () => handleInput(parseFn(el.value), el));
      }
    });
    ...
    numIds.forEach(id => {
      ...
      if (el) {
        el.addEventListener('input', () => {
          ...
          if (!isNaN(val)) handleInput(val, el);
        });
      }
    });
  };

  const bindDualToggles = (toggleIds, propName, globalPropName) => {
    toggleIds.forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      el.addEventListener('change', () => {
        toggleIds.forEach(otherId => {
          const otherEl = document.getElementById(otherId);
          if (otherEl && otherEl !== el) otherEl.checked = el.checked;
        });
        applySelectionPropChange(propName, el.checked, globalPropName);
      });
    });
  };

  bindDualToggles(['param-WORD_BOX_EFFECT_ENABLED', 'param-WORD_BOX_ENABLED'], 'boxEffectEnabled', 'WORD_BOX_ENABLED');
  bindDualControl(['param-WORD_BOX_FILL_COLOR', 'param-GLOBAL_WORD_BOX_FILL_COLOR'], 'boxFillColor', 'WORD_BOX_FILL_COLOR', 'input');
  bindDualCombo(['param-WORD_BOX_FILL_OPACITY', 'param-GLOBAL_WORD_BOX_FILL_OPACITY'], ['num-WORD_BOX_FILL_OPACITY', 'num-GLOBAL_WORD_BOX_FILL_OPACITY'], 'boxFillOpacity', 'WORD_BOX_FILL_OPACITY', parseFloat);
  bindDualControl(['param-WORD_BOX_STROKE_COLOR', 'param-GLOBAL_WORD_BOX_STROKE_COLOR'], 'boxStrokeColor', 'WORD_BOX_STROKE_COLOR', 'input');
  bindDualCombo(['param-WORD_BOX_STROKE_WIDTH', 'param-GLOBAL_WORD_BOX_STROKE_WIDTH'], ['num-WORD_BOX_STROKE_WIDTH', 'num-GLOBAL_WORD_BOX_STROKE_WIDTH'], 'boxStrokeWidth', 'WORD_BOX_STROKE_WIDTH', parseFloat);
  bindDualControl(['param-WORD_BOX_LINE_PATTERN', 'param-GLOBAL_WORD_BOX_LINE_PATTERN'], 'boxLinePattern', 'WORD_BOX_LINE_PATTERN', 'change');
  bindDualCombo(['param-WORD_BOX_ROUNDING', 'param-GLOBAL_WORD_BOX_ROUNDING'], ['num-WORD_BOX_ROUNDING', 'num-GLOBAL_WORD_BOX_ROUNDING'], 'boxRounding', 'WORD_BOX_ROUNDING', parseFloat);
  bindDualCombo(['param-WORD_BOX_PADDING_X', 'param-GLOBAL_WORD_BOX_PADDING_X'], ['num-WORD_BOX_PADDING_X', 'num-GLOBAL_WORD_BOX_PADDING_X'], 'boxPaddingX', 'WORD_BOX_PADDING_X', parseFloat);
  bindDualCombo(['param-WORD_BOX_PADDING_Y', 'param-GLOBAL_WORD_BOX_PADDING_Y'], ['num-WORD_BOX_PADDING_Y', 'num-GLOBAL_WORD_BOX_PADDING_Y'], 'boxPaddingY', 'WORD_BOX_PADDING_Y', parseFloat);
  bindDualCombo(['param-WORD_BOX_SPEED'], ['num-WORD_BOX_SPEED'], 'boxSpeed', 'WORD_BOX_SPEED', parseFloat);
  ...
  function syncInputsFromConfig() {
    ...
    const codeGlitchCharsSync = document.getElementById('param-CODE_GLITCH_CHARS');
    if (codeGlitchCharsSync && cfg.CODE_GLITCH_CHARS !== undefined) {
  ...
  async function fetchWithApiFallback(relPath, options = {}) {
    ...
    const cfgApi = getApiUrl();
    if (cfgApi) candidateBases.push(cfgApi);

    // Si estamos en localhost o 127.0.0.1, probar local y VPS
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      candidateBases.push(window.location.origin + '/artedigitaldata/api');
      candidateBases.push(window.location.origin + '/api');
      candidateBases.push('https://vps-4455523-x.dattaweb.com/artedigitaldata/api');
      candidateBases.push('https://vps-4455523-x.dattaweb.com/api');
    } else {
      // Producción o VPS
      candidateBases.push('/artedigitaldata/api');
    ...
    throw lastErr || new Error(`No se pudo cargar recurso API (${relPath}) desde ningún servidor disponible.`);
  }
    ...
    try {
      ...
      if (effect) {
        ...
        if (effect.config && typeof effect.config === 'object') {
          ...
          delete cfgToApply.TIMELINE_LAYERS;
        ...
        if (flyerWords.length > 0) {
          const trySpawn = () => {
            ...
            if (!hasTimeline && typeof evaluateTimelineAtTime === 'function') {
              evaluateTimelineAtTime(currentTimelineTime);
            }
          };
          trySpawn();
